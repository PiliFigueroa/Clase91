  ...
  return (
      <>
        ...
        <div className='container-cards'>
          {/* {numeros.map((num, index) => <p key={index}>{num}</p>)} */}
          {productos.map(prod => <Card key={prod.id} title={prod.title} img={prod.thumbnail} price={prod.price} shipping={prod.free_shipping} condition={prod.condition} />)}
        </div>
        <Footer />
      </>
  );
